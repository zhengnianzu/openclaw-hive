...
# ============================================================================
# Local Agent Config Preparation
# ============================================================================

def prepare_local_agent_config(sandbox_config, data_config_file: Optional[str] = None) -> None:
    """在 spawn worker 之前把本地 agent 配置文件按 framework 修改到位。
    Args:
        sandbox_config: SandboxConfig 实例
        data_config_file: opencode 分支需要 data_config 里的 agent
            system_prompt
    """
    ...
    if AGENT_FRAMEWORK == "openjiuwen":
        ...
        # 2) 修改user_proxy_model_local_file
        ...
        agent_sp = {agent.get('name', ''): agent.get('system_prompt', '')
...
class OpenClawDistillationTask:
    ...
    async def _copy_agent_config(self, data_config_file) -> None:
        ...
        if AGENT_FRAMEWORK == "opencode":
            # opencode 需要 per-task 的 agent system_prompt, 只能 worker 侧做
            async with _file_lock:
    ...
    async def _upload_traj_to_obs(self, config_file: str) -> bool:
        ...
        purge_clauses = [
            ...
            for cmd in (
                ...
                ([f'rm -rf {os.path.join(ws, ".opencode")}'] if AGENT_FRAMEWORK == "opencode" else [])
            )
        ]
...
def main() -> None:
    ...
    # 在 spawn worker 之前, 把 harness-specific 的本地配置 mutate 集中做一次。
    if AGENT_FRAMEWORK != "opencode":
        prepare_local_agent_config(task_config.sandbox_config)
